...
import { ContentBlock, CmsLink } from '../../types'
import Icon from 'components/theme/plain/Icon'
import { FixMediaPathsInHtml } from 'utilities/functions'

type FooterRPanelRendProp = {
	panelContent: ContentBlock[]
}

export default function FooterRPanelRend({
	panelContent,
}: FooterRPanelRendProp) {
	let richtext = ''
	let thelink = {} as CmsLink
	if (
		panelContent[0] &&
		panelContent[0].fields[0] &&
		panelContent[0].fields[0].html
	) {
		richtext = FixMediaPathsInHtml(panelContent[0].fields[0].html)
